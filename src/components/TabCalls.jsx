...
export default function TabCalls({ calls, readOnly, addCall, updateCall, deleteCall }) {
  ...
  const stats = useMemo(() => {
    ...
    return { agendadas, realizadas, noShow, showRate: agendadas ? (realizadas / agendadas) * 100 : 0 };
  }, [calls]);
  ...
  return <div>
    <div style={{ display:'grid', gridTemplateColumns:'repeat(4,minmax(0,1fr))', gap:12, marginBottom:20 }}>
      {[['Agendadas',stats.agendadas],['Realizadas',stats.realizadas],['No-show',stats.noShow],['Comparecimento',stats.showRate.toFixed(1)+'%']].map(([l,v]) => <div key={l} style={cardStyle}><div style={{color:T.textSec,fontSize:11,textTransform:'uppercase'}}>{l}</div><div style={{fontSize:28,fontWeight:800,marginTop:6}}>{v}</div></div>)}
    </div>
    ...
    <div style={{ ...cardStyle, overflowX:'auto' }}>
      <table style={{width:'100%',borderCollapse:'collapse',minWidth:900}}><thead><tr>{['Lead','Data','Tipo','Produto','Status','Resultado','Próximo passo',''].map(h=><th key={h} style={{textAlign:'left',fontSize:11,color:T.textSec,padding:'10px 8px',borderBottom:`1px solid ${T.border}`}}>{h}</th>)}</tr></thead><tbody>
        {calls.map(c=><tr key={c.id}>
          ...
          <td style={{padding:8}}>{readOnly ? c.status : <select style={inputStyle} value={c.status} onChange={e=>updateCall(c.id,{status:e.target.value})}><option value="agendada">Agendada</option><option value="realizada">Realizada</option><option value="no_show">No-show</option><option value="remarcada">Remarcada</option></select>}</td>
