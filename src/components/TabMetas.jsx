...
export default function TabMetas({ metas, vendas, calls, kpisProsp, readOnly, upsertMeta }){
  ...
  const [form,setForm]=useState({ competencia:monthStart(),meta_receita:0,meta_comissao:0,meta_vendas:0,meta_calls:0,meta_prospects:0,meta_abordagens:0 });

  useEffect(()=>{
    setForm({
      competencia:monthStart(),
      meta_receita:current.meta_receita||0,
      meta_comissao:current.meta_comissao||0,
      meta_vendas:current.meta_vendas||0,
      meta_calls:current.meta_calls||0,
      meta_prospects:current.meta_prospects||0,
      meta_abordagens:current.meta_abordagens||0,
    });
  },[current.id,current.meta_receita,current.meta_comissao,current.meta_vendas,current.meta_calls,current.meta_prospects,current.meta_abordagens]);
